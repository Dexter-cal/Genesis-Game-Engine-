...
impl Default for InstallConfig {
    fn default() -> Self {
        ...
        let install_path = match sys.os_family {
            ...
            _                 => PathBuf::from("/opt/chronoverse"),
        };
...
CV_VERSION="0.1.0"
CV_DOWNLOAD="https://releases.chronoverse.io/linux/$CV_VERSION/chronoverse-linux-x86_64.tar.gz"
...
if command -v curl &> /dev/null; then
    curl -L --progress-bar "$CV_DOWNLOAD" -o /tmp/chronoverse.tar.gz
elif command -v wget &> /dev/null; then
    wget -q --show-progress "$CV_DOWNLOAD" -O /tmp/chronoverse.tar.gz
fi

# Extract
echo "→ Extracting..."
tar -xzf /tmp/chronoverse.tar.gz -C "$INSTALL_DIR"
rm /tmp/chronoverse.tar.gz
...
if [ -d ~/.local/share/applications ]; then
    cat > ~/.local/share/applications/chronoverse.desktop << 'DESKTOP'
[Desktop Entry]
Name=ChronoVerse
Comment=AI-First Game Engine
Exec={install_path}/chronoverse
Icon={install_path}/icons/chronoverse.png
Terminal=false
Type=Application
...
if command -v xdg-mime &> /dev/null; then
    xdg-mime default chronoverse.desktop application/x-chronoverse-project
fi
...
# Create CLI symlink
sudo ln -sf "$INSTALL_DIR/bin/chronoverse-cli" /usr/local/bin/chronoverse-cli 2>/dev/null || \
    echo "Note: Run 'source $SHELL_RC' to use chronoverse-cli"

echo ""
echo "✅ ChronoVerse installed successfully!"
echo ""
echo "→ Launch from applications menu or run:"
echo "  $INSTALL_DIR/chronoverse"
echo ""
echo "→ Or use the CLI:"
echo "  source ~/.bashrc && chronoverse-cli new my-game"
...
if [ "$ARCH" = "arm64" ]; then
    DOWNLOAD_URL="https://releases.chronoverse.io/macos/chronoverse-macos-arm64.dmg"
    echo "Using Apple Silicon build"
else
    DOWNLOAD_URL="https://releases.chronoverse.io/macos/chronoverse-macos-x86_64.dmg"
...
# CLI tool
sudo ln -sf /Applications/ChronoVerse.app/Contents/MacOS/chronoverse-cli /usr/local/bin/chronoverse-cli
...
$DownloadUrl = if ($Arch -eq "ARM64") {
    "https://releases.chronoverse.io/windows/chronoverse-windows-arm64.exe"
} else {
    "https://releases.chronoverse.io/windows/chronoverse-windows-x64.exe"
}
...
New-Item -Path "HKCU:\Software\Classes\ChronoVerseProject\shell\open\command" -Force |
    Set-ItemProperty -Name "(Default)" -Value "`"$InstallPath\chronoverse.exe`" `"%1`""
...
$Shortcut.TargetPath = "$InstallPath\chronoverse.exe"
$Shortcut.IconLocation = "$InstallPath\chronoverse.ico"
$Shortcut.Save()

Write-Host ""
Write-Host "✅ ChronoVerse installed successfully!" -ForegroundColor Green
Write-Host "Launch from Start Menu or run: chronoverse" -ForegroundColor White
"#.to_string()
    }
}
...
impl AutoUpdater {
    ...
    pub fn update_check_url(&self) -> String {
        ...
        format!("https://releases.chronoverse.io/updates/{}/latest.json", channel)
    }
...
impl PackageManager {
    pub fn new(local_repo: PathBuf) -> Self {
        Self {
            ...
            registry_url: "https://registry.chronoverse.io/packages".to_string(),
            local_repo,
        }
    }
