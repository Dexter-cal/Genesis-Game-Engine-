...
// ─── Modifier Stack (Professional Creative Suite) ──────────────────────────
...
pub enum Modifier {
    ...
    Bevel { width: f32, segments: u32 },
    Boolean { operation: BooleanOp, target_entity: String },
    Displace { strength: f32, texture_id: String },
    Decimate { ratio: f32 },
    Solidify { thickness: f32 },
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum BooleanOp { Union, Difference, Intersection }

// ─── Rigging Tree ──────────────────────────────────────────────────────────

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RiggingTree {
    pub root_bone: Bone,
}
...
pub enum BoneConstraint {
    CopyRotation { target: String, influence: f32 },
    LimitRotation { min: [f32; 3], max: [f32; 3] },
    InverseKinematics { target: String, chain_length: u32 },
}
...
impl StudioManager {
    ...
    pub fn generate_studio_qr(&self, session_id: &str, role: QrCodeType) -> String {
        format!("https://studio.genesis.io/join/{}/{:?}", session_id, role)
    }
}
